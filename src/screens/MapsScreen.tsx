    ...
    <div className="flex flex-col h-full relative" onClick={() => setShowSuggestions(false)}>
      ...
      >
        {allFilters.map(p => (
          <button
            key={p.id}
            ref={(el) => { if (el && filter === p.id) { try { el.scrollIntoView({ inline: 'center', block: 'nearest', behavior: 'auto' }); } catch {} } }}
            onClick={(e) => { setFilter(p.id); setSubsVisible(true); (e.currentTarget as HTMLElement).scrollIntoView({ inline: 'center', block: 'nearest', behavior: 'smooth' }); }}
